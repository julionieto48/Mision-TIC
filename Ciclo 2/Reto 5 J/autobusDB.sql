...
CREATE TABLE IF NOT EXISTS `mydb`.`Pasajero` (
  ...
  `Usuario_Nombre_user` VARCHAR(20) NOT NULL,
  PRIMARY KEY (`idPasajero`, `Cedula_pas`, `Usuario_Nombre_user`),
  INDEX `fk_Pasajero_Equipaje1_idx` (`Equipaje_idEquipaje` ASC) VISIBLE,
  INDEX `fk_Pasajero_Usuario1_idx` (`Usuario_Nombre_user` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_Pasajero_Usuario1`
    FOREIGN KEY (`Usuario_Nombre_user`)
    REFERENCES `mydb`.`Usuario` (`Nombre_user`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
